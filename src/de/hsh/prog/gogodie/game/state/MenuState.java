...
public class MenuState extends GameState implements Runnable{
	...
	public void draw(Graphics2D g) {
		...
		Content.drawString(g, options[0], 540, 480, 5, selected[0]);
		Content.drawString(g, options[1], 460, 530, 5, selected[1]);
		Content.drawString(g, options[2], 560, 580, 5, selected[2]);
		Content.drawString(g, options[3], 560, 630, 5, selected[3]);
	}
	...
	public void handleInput() {
		...
		if(mx <= 740 && mx >= 540 && my >= 480 && my <= 520) {
			selected[0] = true;
		}else {
			selected[0] = false;
		}
		
		if(mx <= 820 && mx >= 460 && my >= 530 && my <= 570) {
			selected[1] = true;
		}else {
			selected[1] = false;
		}
		
		if(mx <= 720 && mx >= 560 && my >= 580 && my <= 620) {
			selected[2] = true;
		}else {
			selected[2] = false;
		}
